...
export function DashboardClient({ electionId, candidates, tpsRows, initialTotals, initialGolputTotals, pollUrl, standalone = false }: DashboardClientProps) {
  ...
  return <main className={`mx-auto flex w-full max-w-[1440px] flex-col gap-2 overflow-x-hidden px-2 pt-2 pb-0 md:gap-3 md:px-4 md:pt-3 md:pb-0 ${standalone ? "h-dvh overflow-y-hidden" : "min-h-[calc(100dvh-5.25rem)] md:min-h-[calc(100dvh-6.25rem)]"}`}>
    ...
    <footer className={`-mx-2 mt-auto grid w-[calc(100%+1rem)] shrink-0 grid-cols-[68px_minmax(0,1fr)_68px] gap-3 overflow-hidden border-y-4 border-[#3f73ad] bg-black text-white md:-mx-4 md:min-h-[96px] md:w-[calc(100%+2rem)] md:grid-cols-[68px_minmax(0,1fr)_68px] md:items-center md:gap-5 md:px-5 md:py-3 ${standalone ? "min-h-0 flex-1 items-start px-3 pb-6 pt-3" : "min-h-[210px] items-center px-3 py-6"}`}>
