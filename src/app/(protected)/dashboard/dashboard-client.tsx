...
export function DashboardClient({ electionId, candidates, tpsRows, initialTotals, initialGolputTotals, pollUrl, standalone = false }: DashboardClientProps) {
  ...
  return <main className={`mx-auto flex w-full max-w-[1440px] flex-col gap-2 overflow-x-hidden px-2 pt-2 pb-0 md:gap-3 md:px-4 md:pt-3 md:pb-0 ${standalone ? "public-results h-dvh overflow-y-hidden" : "min-h-[calc(100dvh-5.25rem)] md:min-h-[calc(100dvh-6.25rem)]"}`}>
        ...
        <div key={visibleSlide} className={`tps-slide-enter absolute inset-0 grid grid-cols-3 gap-1.5 md:grid-cols-7 md:gap-2 ${standalone ? "items-stretch" : "items-center"}`}>
          {currentSlide.map((tps) => {
            ...
            return <article key={tps.id} className={`flex min-h-0 min-w-0 flex-col overflow-hidden rounded-xl border border-neutral-300 bg-white text-center shadow-sm md:p-2 ${standalone ? "px-1 py-0.5" : "px-1.5 py-1.5"}`}>
              ...
              <p className={`flex items-center justify-center rounded-md bg-neutral-900 px-1 font-extrabold uppercase text-white md:min-h-8 md:py-1.5 md:text-[10px] ${standalone ? "min-h-6 py-1 text-[7px]" : "min-h-7 py-1.5 text-[8px]"}`}>Tidak Sah <strong className="ml-1 text-xs tabular-nums md:text-sm">{golputValue}</strong></p>
    ...
    <footer className={`-mx-2 mt-auto grid w-[calc(100%+1rem)] shrink-0 grid-cols-[68px_minmax(0,1fr)_68px] gap-3 overflow-hidden border-y-4 border-[#3f73ad] bg-black text-white md:-mx-4 md:min-h-[96px] md:w-[calc(100%+2rem)] md:grid-cols-[68px_minmax(0,1fr)_68px] md:items-center md:gap-5 md:px-5 md:py-3 ${standalone ? "public-footer min-h-0 flex-1 items-start px-3 pb-6 pt-3" : "min-h-[210px] items-center px-3 py-6"}`}>
      ...
      <div className="public-footer-copy min-w-0 text-center uppercase md:-translate-y-[9px]">
      ...
      <Image src="/gaskeun.PNG" width={68} height={68} alt="Logo Gaskeun" className="public-footer-logo h-16 w-16 object-contain" />
    </footer>
  </main>;
}
